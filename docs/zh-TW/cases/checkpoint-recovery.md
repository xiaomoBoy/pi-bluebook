...
description: 用三篇練習材料模擬長任務中斷，通過進度檔案恢復處理，檢查續寫過程是否做到不重不漏。
...
三篇材料都是短小的虛構文本；進度模板明確分開已完成、已處理、失敗和下一步。
...
恢復 工作階段 先讀取磁碟進度，而不是猜測前一個工作階段做到哪裡。[第 13 課](/zh-TW/guide/vps-and-long-running)進一步解釋 VPS 與 tmux 邊界。
