...
退出 Pi，開啟 `index.md` 和 `progress.md`，確認已完成數為 1、清單只有 `article-a.md`，下一步仍指向未處理材料。
...
輸入檔案數、索引條目數、進度中的已完成數都為 3；已處理清單無重複，失敗清單與實際一致。再逐篇核對摘要，不能只比數字。

## 失敗恢復

中斷後先讀進度和現有輸出，不從頭盲跑。發現重複時保留現場，列出重複項及來源後再決定修正；檔案損壞時寫入失敗清單並停止。
