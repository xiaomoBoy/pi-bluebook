...
next: { text: 回到課程, link: /zh-TW/guide/safety }
---
...
先開啟兩份檔案，確認它們只有固定場景和七個空白標題，不包含可執行程式碼。然後顯式關閉 擴充功能（Extension） 和上下文檔案，只提供讀取、搜尋與寫入工具：
...
這些引數不是沙箱；`write` 仍能以當前使用者權限寫檔案。本課材料是無害的固定教學文本，任務只允許寫一個新結果。進入 Pi 後傳送：
