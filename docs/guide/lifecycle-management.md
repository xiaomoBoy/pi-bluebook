...
准备更新前，可以先到[Pi 版本更新记录](/releases/)输入当前版本或目标版本，检查新功能、Breaking Changes、迁移说明和已修复问题。版本档案用于理解差异，实际更新仍按本页的恢复点和验收步骤进行。

::: danger 先记住最容易误判的一点
按照 Pi 官方当前说明，卸载 Pi **不会**自动删除 `~/.pi/agent/`。这个目录可能包含认证信息、会话、设置和已安装 Package。看到 `pi: command not found`，不能证明凭据和历史记录已经从电脑中消失。
:::

本页同时覆盖蓝皮书当前采用的官方安装器和 npm 路线。更新 Pi 本体都可以先使用 `pi update`；卸载时必须回到自己原来的安装方式。Windows 用户看到“普通终端”时，仍应打开 Git Bash。
...
| 暂时不用 Pi，但想保留历史 | 用原安装方式卸载 Pi，保留 `~/.pi/agent/` | 不会自动清理本地数据 |
...
## 5. 按原安装方式卸载 Pi，但保留设置和会话

先用 `/quit` 退出 Pi，再回到普通终端。先根据安装时的记录选择一条卸载路线，不确定时不要把两条命令都运行一遍。

如果通过官方安装器安装，重新运行同一个官方入口，并在菜单中选择卸载：

```bash
curl -fsSL https://pi.dev/install.sh | sh
```

执行前再次确认域名是 `pi.dev`。安装器会检查当前安装并提供对应动作；阅读菜单后只选择卸载。

如果通过 npm 全局安装，使用同一个包管理器卸载：
...
通过任一路线卸载后，`~/.pi/agent/` 默认仍然保留。以后重新安装时，原有设置和会话通常仍可继续使用；重新安装后仍要运行 `pi --version`，并用 `/resume` 实际确认需要的会话是否存在。
...
页面中的安装器、npm 卸载方式和存储位置核验于 2026-09-23。实际使用时仍应以当前官方文档和本机 `pi --help` 为准。
