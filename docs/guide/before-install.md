...
::: info 本课环境
主线按 macOS 编写。Linux 可以使用同样的检查命令；如果没有 `~/Downloads` 目录，就把本课和后续安装、登录步骤中的 `~/Downloads/pi-practice` 统一换成 `~/pi-practice`。Windows 用户不要直接改写本页命令，请改走完整的 [Windows 中文安装路径](/guide/windows-setup)：它会从 Git Bash、Node.js 和练习目录开始，一直带你完成第一次启动。
:::
...
- Linux 的复制、粘贴快捷键随终端而变；可以使用终端菜单或右键菜单，确认粘贴内容完整后再按回车。
...
在 Mac 上，我最推荐使用 [iTerm2](https://iterm2.com/)；如果还没安装，系统自带的“终端”也可以完成本书全部入门步骤。按 `Command + Space`，输入“iTerm”或“终端”，再按回车。看到带有光标的窗口后，输入下面这行并按回车。Linux 读者打开自己发行版中的终端，执行相同的检查命令。
