...
2026 開放學習版的讀者對象、閱讀順序、基本主張和內容邊界已經確定；網站仍會持續校訂錯字、失效命令、連結和核驗日期。當前導論、序章、五個模組、14 課和 7 個配套案例，已經形成從理解 Pi、完成第一次任務、管理工作階段與上下文，到擴充、分工、長任務和安全驗收的完整學習路徑。需要真實系統介面的進階效果會明確標出，不用文字或偽圖代替實測。
...
| 模組四 · 10至12 | [擴充自己的 Pi](docs/zh-TW/guide/skills-extensions-packages.md) | 區分 技能（Skill）、擴充功能（Extension） 和 套件，理解 擴充功能 設計與子代理（Subagent） 分工 |
| 模組五 · 13至14 | [穩定工作流](docs/zh-TW/guide/vps-and-long-running.md) | 處理長時間任務，建立權限、隔離、恢復和驗收意識 |
| 實作案例 | [案例庫](docs/zh-TW/cases/index.md) | 用 7 個固定材料案例練習檔案任務、壓縮對照、技能、擴充功能、分工、恢復和安全審閱 |
...
| 小墨札記 | [寫在藍皮書之外](docs/zh-TW/journey/index.md) | 單獨保留個人感悟、踩坑記錄與 98 條推文件案 |
...
當前仍未驗收的是 Windows 真實介面截圖，以及 擴充功能 桌面通知的完整系統證據；取得對應實機後再補入，不用模擬圖或終端機文字代替。新的個人實踐只有在來源和操作重新核驗後才會進入本版。
...
開啟終端機顯示的本地地址即可預覽；修改 `docs/` 下的檔案後，頁面會自動更新。停止預覽時，在該終端機按 `Ctrl+C`。
...
構建結果生成在 `docs/.vitepress/dist/`，無需提交到倉庫。`docs:preview` 預覽的是上一次構建結果，內容有變化時請重新構建。
