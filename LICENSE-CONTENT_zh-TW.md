# 內容許可

除非頁面或素材旁另有說明，本倉庫中的網站程式碼、原創書稿、推文整理稿和原創圖片
統一採用 [MIT License](LICENSE)。

你可以複製、使用、修改、釋出和分發這些原創內容，也可以用於商業用途。轉載、改編或
分發本專案的全部或重要部分時，需要保留原版權宣告和 MIT License。

第三方商標、截圖、引用和其他第三方素材不自動包含在本許可中，其權利歸各自權利人所有。

## Pi 官方版本記錄

`docs/.vitepress/data/pi-releases.json` 中的英文版本明細由
[Pi Coding Agent 官方 Changelog](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/CHANGELOG.md)
生成。上游專案採用 [MIT License](https://github.com/earendil-works/pi/blob/main/LICENSE)：

> Copyright (c) 2025 Mario Zechner

本倉庫保留上述版權聲明和上游許可證連結。版本檔案頁面的中文分類、檢索組織和關鍵節點說明屬於本專案原創整理，採用本倉庫 MIT License。

## Earendil 官方授權譯文

`docs/translations/` 中十一篇 Earendil 文章的中文譯文及適配部分，
經 Earendil 授權按
[Creative Commons Attribution 4.0 International](https://creativecommons.org/licenses/by/4.0/deed.zh-hant)
釋出。頁面保留原文標題、作者、釋出日期、原文連結及授權說明；英文原文版權歸 Earendil 所有。
原文配圖隨對應文章授權使用，頁面保留原攝影者、製圖方或專案來源署名；這些圖片不因進入本倉庫而自動適用網站原創內容的 MIT License。

## 繁體中文版

`docs/zh-TW/`、`docs/public/examples-tw/`、根目錄 `README_zh-TW.md` 等繁體中文內容，
由本倉庫 MIT 許可的簡體中文原創內容經轉換生成，屬於 MIT 許可允許的改編，
同樣採用 [MIT License](LICENSE)。轉載、改編或分發時同樣需要保留原版權宣告和許可證。

`docs/zh-TW/translations/` 為簡中授權譯文的繁體轉換版，與原譯文同樣按
[Creative Commons Attribution 4.0 International](https://creativecommons.org/licenses/by/4.0/deed.zh-hant)
釋出；頁面保留授權說明、出處與“繁體轉換版”標註，英文原文版權仍歸 Earendil 所有。

繁體中文版與簡體中文版在同一倉庫維護，MIT 權利與保留宣告要求相同。
